...
describe('Lottery Contract' ,  ()=>{
	...
	it('require a miniium amount of ether to send ',async ()=>{
		try{
			await lottery.methods.send().send({
				from: accounts[0],
				value: 0
	...
	it('only manager can  callpick Winner ',async ()=>{
		try{
	...
	it('send money to winner and resets the players array ',async ()=>{
		try{
			...
			assert(difference> web3.utils.toWel('1.8','ether'));
		} catch (err){
			assert(err);
		}
	});
